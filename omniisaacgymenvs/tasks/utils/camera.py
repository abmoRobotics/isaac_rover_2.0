...
class Camera():
    def __init__(self, device, shift):
        ...
        self.map_indices, self.triangles, self.vertices = self._load_triangles_with_indices()
        #TODO Refeactor this
    ...
    def get_depths(self, positions, rotations):
        ...
        triangles,triangles_with_indices = self._height_lookup(self.map_values,depth_points,self.horizontal,self.shift)

        #self._plot_mesh(triangles_with_indices.flatten())
        ...
        return output_distances, output_pt, sources, self.m, self.n, self.k, self.k_after_check, t,self.a,self.b,self.c,self.d,self.g,self.triangles11111,self.sourcesLong
    ...
    def _ray_distance(self, sources: torch.Tensor, directions: torch.Tensor, triangles: torch.Tensor):
        ...
        d = -torch.nn.functional.normalize(directions) # 4000 x 3 # not checked
        
 
        a = triangles[:,2]      # 4000 x 3 # checked 
        b = triangles[:,1] - a  # 4000 x 3 # checked 
        c = triangles[:,0] - a  # 4000 x 3 # checked

        g = sources - a     # 4000 x 3 # not checked but i dont think this is it
       ...
       ## det2=torch.mm(torch.transpose(kk,0,1),d.float())
        ...
        n = torch.where(det == 0.0,-99.0,nn.double())
        ...
        m = torch.where(det == 0.0,-99.0,mm.double()) 
        kk = b.half().cross(c.half())
        ...
        k = torch.where(det == 0.0,-99.0,kk.double()) 
       ...
       # pt = torch.where(k_after_check)
        ...
        self.n = n
        self.k = k
        self.k_after_check = k_after_check
        self.b = b
        self.c = c
        self.d = d
        self.triangles11111 = triangles
    ...
    def _heightmap_distribution(self, x_limit=1.12, y_limit=1.8, square=False, y_start=0.5, delta=0.2, front_heavy=0, plot=False):
    ...
    def _height_lookup(self, triangle_matrix: torch.Tensor, depth_points: torch.Tensor, horizontal_scale, shift):
        ...
        triangles_with_indices = self.map_indices[x, y]
...
#print(cam.get_depths_test(source,direction))
# #print(source.shape)
...
rotations2 = torch.tensor([
        ...
        [-0.0706, -0.0316,  0.0137]], device='cuda:0')
...
#output_distances, output_pt, sources, m,n,k,k_after_check, t,a,b,c,d,g,triangleslong,sourcesLong = cam.get_depths(pos2,rotations2)
# pos5 = torch.tensor([
